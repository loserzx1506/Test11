...
async function main() {
  try {
    ...
    // Xác định chế độ proxy từ config.yaml
    const useProxyConfig = config.general && config.general.use_proxy;
...
async function processBatch(privateKeys, proxies, batchIndex, startIndex) {
  ...
  for (let i = 0; i < privateKeys.length; i++) {
    ...
    try {
      // Bọc toàn bộ processAccount bằng try/catch cấp cao nhất
      try {
        await processAccount(privateKey, proxy, threadWalletId, threadId, globalWalletIndex);
      } catch (error) {
        logger.error(`Thread ${threadId}: Unhandled error in processAccount for account ${i + 1}/${privateKeys.length}: ${error && error.stack ? error.stack : error}`, { walletIndex: threadWalletId });
      }
    } catch (error) {
      logger.error(`Thread ${threadId}: Error processing account ${i + 1}/${privateKeys.length}: ${error && error.stack ? error.stack : error}`, { walletIndex: threadWalletId });
    }
...
async function processAccount(privateKey, proxy, walletIndex, threadId, globalWalletIndex) {
  ...
  if (config.tasks.onchain.swap.enabled) {
    ...
    if (remainingCount > 0) {
      ...
      for (let i = 0; i < remainingCount; i++) {
        // Random 1 pair mỗi lần swap
        const randomIndex = Math.floor(Math.random() * taskConfig.pairs.length);
        const pair = taskConfig.pairs[randomIndex];
  ...
  if (config.tasks.onchain.liquidity.enabled) {
    ...
    if (remainingCount > 0) {
      ...
      for (let i = 0; i < remainingCount; i++) {
        // Random 1 pair mỗi lần add liquidity
        const randomIndex = Math.floor(Math.random() * taskConfig.pairs.length);
        const pair = taskConfig.pairs[randomIndex];
...
// Global error handler: không bao giờ để chương trình dừng lại vì lỗi không bắt được
...
main().catch(error => {
  ...
  // KHÔNG exit nữa để chương trình không bao giờ dừng lại
});
